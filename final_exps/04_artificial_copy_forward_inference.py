...
def is_clearml_remote_run() -> bool:
    """Return True when this process is being launched by a ClearML agent."""
    return bool(
...
def clearml_config_from_args(args: argparse.Namespace) -> dict[str, Any]:
    """Convert argparse values into a ClearML-serializable dictionary."""
...
def args_from_clearml_config(
    original_args: argparse.Namespace,
    config: dict[str, Any],
) -> argparse.Namespace:
    """Rehydrate typed argparse values after ClearML overrides the config."""
    values = vars(original_args).copy()
    ...
    for key in nullable_path_fields:
        raw = values.get(key)
        values[key] = Path(raw) if raw else None
    for key in path_fields:
        raw = values.get(key)
        if raw:
            values[key] = Path(raw)

    raw_run_configs = values.get("run_config") or []
    if isinstance(raw_run_configs, str):
        raw_run_configs = [raw_run_configs]
    values["run_config"] = [Path(x) for x in raw_run_configs]

    bool_fields = {
        "allow_baseline_mismatch",
        "reuse_perturbed_sequences",
        "enable_clearml",
        "execute_remotely",
        "clearml_upload_artifacts",
    }
...
def validate_required_args(args: argparse.Namespace) -> None:
    ...
    if args.dataset_config is None:
        missing.append("--dataset-config")
    if not args.run_config:
        missing.append("--run-config")
    if args.baseline_predictions is None:
        missing.append("--baseline-predictions")
    if missing:
        raise ValueError(
            "Missing required experiment arguments after ClearML synchronization: "
            + ", ".join(missing)
        )


def init_clearml_and_sync(
    args: argparse.Namespace,
) -> tuple[Any | None, argparse.Namespace]:
    """Initialize ClearML and restore the saved launch configuration remotely.

    This deliberately does not depend on ClearML's argparse auto-patching. The
    local launcher stores every CLI value in a connected dictionary. When the
    agent restarts this script without the original argv, task.connect mutates
    that dictionary with the values saved in the task, after which Paths and
    booleans are reconstructed explicitly.
    """
    remote_agent_run = is_clearml_remote_run()
    ...
    connected = clearml_config_from_args(args)
    task.connect(connected, name="copy_forward_cli")
    args = args_from_clearml_config(args, connected)
    validate_required_args(args)
    ...
    if not remote_agent_run and args.clearml_docker_image.strip():
