...
def topk_metrics(
    ...
    При одинаковых risk scores порядок детерминирован:
        1. pred_proba по убыванию;
        2. tie_breaker по возрастанию.

    Для prediction-level анализа передавайте example_id как tie_breaker.
    """
    y_true = np.asarray(y_true).astype(int)
    y_prob = np.asarray(y_prob).astype(float)
    ...
    if tie_breaker is None:
        tie_breaker = np.arange(n, dtype=np.int64)
    else:
        tie_breaker = np.asarray(tie_breaker)
        if len(tie_breaker) != n:
            raise ValueError("tie_breaker must have the same length as y_true")

    order = np.lexsort((tie_breaker, -y_prob))
    rows = []
        ...
        rows.append(
            {
                ...
                "event_capture": (
                    float(events / n_pos) if n_pos > 0 else np.nan
                ),
                "base_event_rate": base_rate,
                "n": int(n),
                "n_positive": int(n_pos),
