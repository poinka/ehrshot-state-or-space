...
def progress_iter(iterable, enabled: bool, **kwargs):
    """
    Обертка над tqdm с fallback.

    Если tqdm недоступен или progress отключен, возвращает обычный iterable.
    Это важно для remote/ClearML запусков: отсутствие tqdm не должно ломать обучение.
    """
    if not enabled:
        return iterable
...
def maybe_set_postfix(progress_obj, **kwargs) -> None:
    """
    Безопасно обновляет postfix у tqdm progress bar.
    Для обычного iterable просто ничего не делает.
    """
    set_postfix = getattr(progress_obj, "set_postfix", None)
...
def build_checkpoint_filename(
    ...
    """
    Единое имя checkpoint-файла.

    Важно:
        Имя должно совпадать с тем, как checkpoint сохраняется в run_one()
        и как строится remote MinIO/S3 путь.
    """
    return "_".join(
        [
...
def build_local_checkpoint_path(
    ...
    ckpt_filename = build_checkpoint_filename(run_cfg, seed)
...
def iter_local_checkpoint_candidates(
    ...
    """
    Возвращает локальные пути, где может лежать checkpoint.

    Проверяем:
        1. основной путь внутри текущего output_dir;
        2. дополнительную локальную папку из --local-checkpoint-dir, если она задана.

    --local-checkpoint-dir может быть:
        - корнем прошлого output_dir;
        - папкой checkpoints;
        - папкой с layout <run_set>/<task>/<model_family>/<checkpoint>.pt.
    """
    filename = build_checkpoint_filename(run_cfg, seed)

    candidates: list[Path] = [
        build_local_checkpoint_path(
    ...
    extra_root = getattr(args, "local_checkpoint_dir", None)

    if extra_root:
        extra_root = Path(extra_root)

        layout_tail = (
            Path(safe_path_part(args.run_set))
            / safe_path_part(run_cfg["task"])
            / safe_path_part(run_cfg["model_family"])
            / filename
        )

        # Если передали корень output_dir.
        candidates.append(extra_root / "checkpoints" / layout_tail)

        # Если передали саму папку checkpoints.
        candidates.append(extra_root / layout_tail)
...
def find_existing_local_checkpoint(
    run_cfg: dict[str, Any],
    seed: int,
    args: argparse.Namespace,
) -> Path | None:
    """
    Ищет checkpoint локально.

    Специально НЕ делаем широкий glob по всем подпапкам, потому что можно
    случайно найти checkpoint от другого run_set/max_len и неправильно
    пропустить текущий запуск.
    """
    for path in iter_local_checkpoint_candidates(run_cfg, seed, args):
        if path.exists():
            return path
...
def build_run_stem(run_cfg: dict[str, Any], seed: int) -> str:
    return (
        f"{run_cfg['task']}__"
...
def expected_run_output_files(
    run_cfg: dict[str, Any],
    seed: int,
    output_dir: Path,
) -> list[Path]:
    stem = build_run_stem(run_cfg, seed)
    return [
        Path(output_dir) / f"{stem}__metrics.csv",
        Path(output_dir) / f"{stem}__heldout_predictions.csv",
        Path(output_dir) / f"{stem}__tuning_predictions.csv",
        Path(output_dir) / f"{stem}__topk.csv",
        Path(output_dir) / f"{stem}__history.csv",
        Path(output_dir) / f"{stem}__platt_calibrator.json",
        Path(output_dir) / f"{stem}__config.json",
    ]


def run_outputs_complete(
    run_cfg: dict[str, Any],
    seed: int,
    output_dir: Path,
) -> bool:
    return all(
        path.exists()
        for path in expected_run_output_files(run_cfg, seed, output_dir)
    )


def download_file_from_minio(remote_url: str, local_path: Path) -> bool:
    if not remote_url:
        return False
    try:
        from clearml import StorageManager
        ...
        if not cached_value:
            return False
        cached = Path(cached_value)
        if cached.is_dir():
            matches = list(cached.rglob(local_path.name))
            if len(matches) != 1:
                return False
            cached = matches[0]
        ...
        print(f"REMOTE RESUME: could not download {remote_url}: {exc!r}")
        return False


def try_restore_complete_run_from_remote(
    run_cfg: dict[str, Any],
    seed: int,
    args: argparse.Namespace,
) -> bool:
    ...
    remote_ckpt = build_sequence_checkpoint_remote_url(
    ...
    pairs = [(remote_ckpt, local_ckpt), *zip(remote_outputs, local_outputs)]
    restored = []
    for remote_url, local_path in pairs:
        if local_path.exists():
            restored.append(True)
        else:
            restored.append(download_file_from_minio(remote_url, local_path))

    ok = all(restored)
    if ok:
        print(
            "REMOTE RESUME: restored checkpoint and all per-run outputs for "
            f"{run_cfg['task']} / {run_cfg['compression_version']} / seed={seed}"
        )
    return ok
...
def should_skip_run(
    ...
    """
    Resume пропускает запуск только когда одновременно существуют:
      - локальный checkpoint;
      - все per-run metrics/predictions/history/calibration files.

    Один remote checkpoint без predictions не является достаточным основанием
    для skip: иначе aggregate results будут неполными.
    """
    if not getattr(args, "resume", False):
        return False

    local_ckpt = find_existing_local_checkpoint(run_cfg, seed, args)
    outputs_ok = run_outputs_complete(run_cfg, seed, args.output_dir)

    if local_ckpt is not None and outputs_ok:
        print(f"RESUME: complete run found, skipping: {local_ckpt}")
        return True

    if local_ckpt is not None and not outputs_ok:
        print(
            "RESUME: checkpoint exists but per-run outputs are incomplete; "
            "trying to restore missing files from storage."
        )

    if try_restore_complete_run_from_remote(run_cfg, seed, args):
        return True
...
def upload_output_files(
    ...
    for path in paths:
        ...
        try:
            ...
            relative = path.name
        remote_url = f"{remote_prefix.rstrip('/')}/{relative}"
        uploaded = upload_file_to_minio(path, remote_url)
        rows.append(
            {
                "local_path": str(path),
                "remote_url": uploaded or remote_url,
                "upload_ok": bool(uploaded),
            }
        )
    return pd.DataFrame(rows)


def upload_output_tree(local_root: Path, remote_prefix: str) -> pd.DataFrame:
    local_root = Path(local_root)
    return upload_output_files(
        [p for p in sorted(local_root.rglob("*")) if p.is_file()],
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="Корневая локальная папка для model checkpoints.",
    )
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Дополнительная локальная папка, где искать checkpoints при --resume. "
            "Можно передать корень прошлого output_dir или саму папку checkpoints."
        ),
    )
    ...
    parser.add_argument(
        "--no-progress-bars",
        action="store_true",
        help="Отключить tqdm progress bars для train/eval loops.",
    )
    parser.add_argument(
        ...
        default=50,
        help=(
            "Fallback print-прогресс каждые N batches, если tqdm недоступен. "
            "0 отключает fallback print."
        ),
    )
...
def make_default_token_numeric_stats(vocab_size: int) -> tuple[np.ndarray, np.ndarray, pd.DataFrame]:
    """
    Быстрый default для code-only моделей.

    Для numeric_on=False numeric_features все равно создаются в collate_fn,
    но модель их не использует. Поэтому сканировать весь train split ради
    token-specific mean/std не нужно.
    """
    mean = np.zeros(vocab_size, dtype=np.float32)
    std = np.ones(vocab_size, dtype=np.float32)
    stats_df = pd.DataFrame(
        {
            "token_id": np.arange(vocab_size),
            "numeric_count_train": np.zeros(vocab_size, dtype=int),
            "numeric_mean_train": mean,
            "numeric_std_train": std,
            "has_enough_numeric_stats": np.zeros(vocab_size, dtype=bool),
            "stats_source": "default_numeric_off",
        }
    )
    return mean, std, stats_df


def get_or_compute_token_numeric_stats(
    df_train: pd.DataFrame,
    vocab_size: int,
    run_cfg: dict[str, Any],
    args: argparse.Namespace,
    ...
    """
    Возвращает token numeric mean/std.

    Оптимизация для long-context runs:
      1. Для code-only models numeric stats не нужны — не сканируем train.
      2. Для numeric_on=True stats не зависят от seed, поэтому кэшируем их
         по task/compression_version/numeric_min_count/vocab_size.
    """
    if not bool(run_cfg["numeric_on"]):
        print("[stage] numeric_on=False -> skip token numeric stats scan", flush=True)
        return make_default_token_numeric_stats(vocab_size)

    cache_dir = Path(args.output_dir) / "numeric_stats_cache"
    cache_dir.mkdir(parents=True, exist_ok=True)
    cache_stem = "__".join(
        [
            safe_path_part(run_cfg["task"]),
            safe_path_part(run_cfg["compression_version"]),
            f"vocab{int(vocab_size)}",
            f"mincount{int(args.numeric_min_count)}",
        ]
    )
    npz_path = cache_dir / f"{cache_stem}.npz"
    csv_path = cache_dir / f"{cache_stem}.csv"

    if npz_path.exists() and csv_path.exists():
        print(f"[stage] loading cached token numeric stats: {npz_path}", flush=True)
        arr = np.load(npz_path)
        token_mean = arr["token_mean"].astype(np.float32)
        token_std = arr["token_std"].astype(np.float32)
        stats_df = pd.read_csv(csv_path)
        stats_df["stats_source"] = "cache"
        return token_mean, token_std, stats_df

    print("[stage] computing token numeric stats on train split; this may take a while for long sequences", flush=True)
    t0 = time.perf_counter()
    token_mean, token_std, stats_df = compute_token_numeric_stats(
        df_train=df_train,
        vocab_size=vocab_size,
        min_count=args.numeric_min_count,
    )
    elapsed = time.perf_counter() - t0
    stats_df["stats_source"] = "computed"
    stats_df["stats_compute_seconds"] = float(elapsed)

    np.savez_compressed(npz_path, token_mean=token_mean, token_std=token_std)
    stats_df.to_csv(csv_path, index=False)
    print(f"[stage] saved token numeric stats cache: {npz_path} ({elapsed:.1f}s)", flush=True)
    return token_mean, token_std, stats_df
...
def train_model(
    ...
    # Не итерируемся по train DataLoader только ради labels.
    # Для long-context runs это делает скрытый дорогой collate-pass до первого progress bar.
    ...
    progress_enabled = not getattr(args, "no_progress_bars", False)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        train_loader = progress_iter(
            loaders["train"],
            enabled=progress_enabled,
            desc=f"train epoch {epoch:02d}/{args.epochs}",
        ...
        for batch_idx, batch in enumerate(train_loader, start=1):
            ...
            mean_loss_so_far = float(np.mean(losses))
            maybe_set_postfix(
                train_loader,
                loss=f"{loss_value:.4f}",
                avg=f"{mean_loss_so_far:.4f}",
            )

            fallback_every = int(getattr(args, "progress_every_n_batches", 50))
            if (
                not progress_enabled
                and fallback_every > 0
                and (batch_idx % fallback_every == 0 or batch_idx == n_train_batches)
            ):
                elapsed = time.perf_counter() - epoch_start
                examples_done = batch_idx * int(loaders["train"].batch_size or 1)
                print(
                    f"epoch={epoch:02d}/{args.epochs} "
                    f"batch={batch_idx}/{n_train_batches} "
                    f"avg_loss={mean_loss_so_far:.4f} "
                    f"elapsed={elapsed:.1f}s "
                    f"approx_examples={examples_done}"
                )

        tuning_pred = run_inference(
            model,
            loaders["tuning"],
            device,
            progress_enabled=progress_enabled,
            progress_desc=f"eval tuning epoch {epoch:02d}",
        )
        ...
        print(
            ...
            f"epoch_seconds={time.perf_counter() - epoch_start:.1f} "
...
def evaluate_with_platt(
    ...
    tuning = run_inference(
        model,
        loaders["tuning"],
        device,
        progress_enabled=progress_enabled,
        progress_desc="final eval tuning",
    )
    heldout = run_inference(
        model,
        loaders["held_out"],
        device,
        progress_enabled=progress_enabled,
        progress_desc="final eval held_out",
    )
    ...
    tuning_probs = {
        "raw": sigmoid_np(tuning["logits"]),
        "platt": apply_platt(calibrator, tuning["logits"]),
    }

    tuning_frames = []
    for calibration, pred_proba in tuning_probs.items():
            ...
            pd.DataFrame(
                {
                    ...
                    "seed": int(seed),
                    "split": "tuning",
                    "example_id": tuning["example_id"].astype(int),
                    "subject_id": tuning["subject_id"].astype(int),
                    "y_true": tuning["y_true"].astype(int),
                    "pred_proba": pred_proba.astype(float),
                    "logit": tuning["logits"].astype(float),
                }
            )[REQUIRED_PREDICTION_COLUMNS]
        )

    tuning_stem = build_run_stem(run_cfg, seed)
    pd.concat(tuning_frames, ignore_index=True).to_csv(
        args.output_dir / f"{tuning_stem}__tuning_predictions.csv",
        index=False,
    )
    with open(
        args.output_dir / f"{tuning_stem}__platt_calibrator.json",
        "w",
        encoding="utf-8",
    ) as f:
        json.dump(
            {
                ...
                "classes": [int(x) for x in calibrator.classes_.tolist()],
                "fit_split": "tuning",
                "applied_split": "held_out",
            },
            f,
            ensure_ascii=False,
            indent=2,
    ...
    for calibration, pred_proba in probs.items():
        ...
        topk = topk_metrics(
            heldout["y_true"],
            pred_proba,
            tie_breaker=heldout["example_id"],
        )
...
def build_config_row(
    ...
    return {
        ...
        "results_s3_prefix": str(args.results_s3_prefix),
...
def run_one(
    ...
    print("[stage] loading sequence examples", flush=True)
    t0 = time.perf_counter()
    ...
    print(f"[stage] loaded examples: rows={len(df)} in {time.perf_counter() - t0:.1f}s", flush=True)

    print("[stage] loading vocab", flush=True)
    vocab = load_vocab(
        sequence_data_dir=args.sequence_data_dir,
        task=run_cfg["task"],
        compression_version=run_cfg["compression_version"],
    ...
    print(f"[stage] vocab_size={vocab_size}", flush=True)

    print("[stage] preparing train split", flush=True)
    train_df = df[df["split"] == "train"].copy().reset_index(drop=True)

    token_mean, token_std, token_stats = get_or_compute_token_numeric_stats(
        df_train=train_df,
        vocab_size=vocab_size,
        run_cfg=run_cfg,
        args=args,
    ...
    print("[stage] creating model", flush=True)
    model = make_model(
        run_cfg=run_cfg,
        vocab_size=vocab_size,
        args=args,
    ...
    print("[stage] final evaluation + Platt calibration", flush=True)
    ...
    config_json_path = args.output_dir / f"{stem}__config.json"
    with config_json_path.open("w", encoding="utf-8") as f:
        json.dump(config_row, f, ensure_ascii=False, indent=2)
    ...
    per_run_files = [
        ...
        history_path,
        token_stats_path,
        args.output_dir / f"{stem}__tuning_predictions.csv",
        args.output_dir / f"{stem}__platt_calibrator.json",
        config_json_path,
    ]
    per_run_upload_manifest = upload_output_files(
        per_run_files,
        local_root=args.output_dir,
        remote_prefix=args.results_s3_prefix,
    )
    if len(per_run_upload_manifest):
        per_run_upload_manifest.to_csv(
            args.output_dir / f"{stem}__upload_manifest.csv",
            index=False,
        )
...
def build_clearml_config(args: argparse.Namespace) -> dict[str, Any]:
    ...
    for key in ["run_config", "sequence_data_dir", "output_dir", "checkpoint_dir", "local_checkpoint_dir"]:
...
def _read_csv_parts(paths: list[Path]) -> pd.DataFrame:
    parts = [pd.read_csv(path) for path in paths if path.exists()]
    if not parts:
        return pd.DataFrame()
...
def rebuild_aggregates_from_per_run_files(output_dir: Path) -> dict[str, pd.DataFrame]:
    output_dir = Path(output_dir)
    results = _read_csv_parts(sorted(output_dir.glob("*__metrics.csv")))
    predictions = _read_csv_parts(
        sorted(output_dir.glob("*__heldout_predictions.csv"))
    )
    topk = _read_csv_parts(sorted(output_dir.glob("*__topk.csv")))
    history = _read_csv_parts(sorted(output_dir.glob("*__history.csv")))

    config_rows = []
    for path in sorted(output_dir.glob("*__config.json")):
        with path.open("r", encoding="utf-8") as f:
            config_rows.append(json.load(f))
    configs = pd.DataFrame(config_rows).drop_duplicates() if config_rows else pd.DataFrame()
    ...
    filenames = {
        "results": "sequence_multiseed_results.csv",
        "predictions": "sequence_multiseed_heldout_predictions.csv",
        "topk": "sequence_multiseed_topk.csv",
        "history": "sequence_multiseed_history.csv",
        "configs": "sequence_multiseed_configs.csv",
    }
...
def main() -> None:
    ...
    def _load_existing_csv(filename: str) -> pd.DataFrame:
        ...
        if args.resume and path.exists():
            ...
            print(f"RESUME: loaded aggregate {path} rows={len(frame)}")
            return frame
        return pd.DataFrame()

    existing_results = _load_existing_csv("sequence_multiseed_results.csv")
    existing_predictions = _load_existing_csv(
        "sequence_multiseed_heldout_predictions.csv"
    )
    existing_topk = _load_existing_csv("sequence_multiseed_topk.csv")
    existing_history = _load_existing_csv("sequence_multiseed_history.csv")
    existing_configs = _load_existing_csv("sequence_multiseed_configs.csv")

    all_results = [existing_results] if not existing_results.empty else []
    all_predictions = (
        [existing_predictions] if not existing_predictions.empty else []
    )
    all_topk = [existing_topk] if not existing_topk.empty else []
    all_history = [existing_history] if not existing_history.empty else []
    all_token_stats = []
    all_configs = (
        existing_configs.to_dict(orient="records")
    ...
    aggregate_tables = rebuild_aggregates_from_per_run_files(args.output_dir)
    results_df = aggregate_tables["results"]
    predictions_df = aggregate_tables["predictions"]
    topk_df = aggregate_tables["topk"]
